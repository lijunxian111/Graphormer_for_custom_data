...
class Graphormer(pl.LightningModule):
    def __init__(
        ...
        if dataset_name == "ZINC":
            ...
            self.atom_encoder = nn.Linear(129, hidden_dim, bias=False) #改成linear层
            self.edge_encoder = nn.Embedding(1024, num_heads, padding_idx=0) #512*3+1
            self.edge_type = edge_type
            if self.edge_type == "multi_hop":
                self.edge_dis_encoder = nn.Embedding(128 * num_heads * num_heads, 1)
            self.rel_pos_encoder = nn.Embedding(1024, num_heads, padding_idx=0)
            self.in_degree_encoder = nn.Embedding(1024, hidden_dim, padding_idx=0)
            self.out_degree_encoder = nn.Embedding(1024, hidden_dim, padding_idx=0)
    ...
    def forward(self, batched_data, perturb=None):
        ...
        #in_degree = torch.arange(636, -1, -1).view(1,-1).long()
        #out_degree = torch.cat([torch.arange(1, 637), torch.tensor([0])]).view(1,-1).long()
        ...
        indeg_embeds = self.in_degree_encoder(in_degree)
        node_feature = (
            node_feature
            + indeg_embeds
        ...
        if self.dataset_name == "PCQM4M-LSC":
            ...
            output = self.downstream_out_proj(output[:, 1, :]) #去掉虚拟token的第一个token
        else:
    ...
    def training_step(self, batched_data, batch_idx):
        ...
        if self.dataset_name == "ogbg-molpcba":
            ...
            loss = self.loss_fn(y_hat, y_gt)
            #loss = self.loss_fn(y_hat, y_gt[train_mask])
    ...
    def validation_step(self, batched_data, batch_idx):
        ...
        if self.dataset_name in ["PCQM4M-LSC", "ZINC"]:
            ...
            loss = self.loss_fn(y_hat[1:638][val_mask], y_gt[val_mask])
            #loss = self.loss_fn(y_hat, y_gt[val_mask])
            self.log("valid_cross_entropy", loss, sync_dist=True)
        elif self.dataset_name == "CUSTOM":
            y_hat = self(batched_data)
            y_pred = torch.argmax(y_hat, dim=1)
            y_gt = batched_data.y.view(-1)
    ...
    def validation_epoch_end(self, outputs):
        ...
        if self.dataset_name == "ogbg-molpcba":
            ...
            print(f"acc: {torch.sum(y_true==y_pred)/y_true.shape[0]}") #打印准确率
            try:
                self.log(
                    "valid_" + self.metric,
    ...
    def test_epoch_end(self, outputs):
        y_pred = torch.cat([i["y_pred"] for i in outputs])
        y_true = torch.cat([i["y_true"] for i in outputs])
        pred_labels = torch.cat([i["pred_labels"] for i in outputs])
        if self.dataset_name == "PCQM4M-LSC":
            ...
            exit(0)
        input_dict = {"y_true": y_true, "y_pred": y_pred}
        print(f"acc: {torch.sum(y_true.view(-1) == pred_labels) / y_true.shape[0]}")  # 打印准确率
        self.log(
            "test_" + self.metric,
            self.evaluator.eval(input_dict)[self.metric],
            sync_dist=True,
    ...
    def add_model_specific_args(parent_parser):
        ...
        parser.add_argument("--checkpoint_path", type=str, default=r"") #改成自己的路径!!!
        parser.add_argument("--warmup_updates", type=int, default=4000)
        parser.add_argument("--tot_updates", type=int, default=40000)
